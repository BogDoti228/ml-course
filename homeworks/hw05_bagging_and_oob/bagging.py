...
class SimplifiedBaggingRegressor:
    ...
    def predict(self, data):
        ...
        '''
        predictions = [np.mean([model.predict([x]) for model in self.models_list]) for x in data]
    ...
    def _get_averaged_oob_predictions(self):
        ...
        self.oob_predictions = np.array([np.mean(p) if p else None for p in self.list_of_predictions_lists])
    ...
    def OOB_score(self):
        ...
        return ((self.target[valid_indices] - self.oob_predictions[valid_indices])** 2).mean()